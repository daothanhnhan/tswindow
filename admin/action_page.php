...
class action_page extends library{
	  ...
	  //   	if(isset($_FILES['fileUpload2']) && $_FILES['fileUpload2']['name'] != ""){
			...
			if($insert != ''){
				...
				$this->Insert_1('image',
									...
									array(&$img_name, &$insert),
									'si'
					);
				/*---------------------------------------*/
				if (!empty($anh_tv)) {

					foreach ($anh_tv as $key => $val) {
   ...
   //          }
			...
			///////////////////////
			$this->Delete_1('image','',array(&$product_id),array('product_id'),'i');
			/*---------------------------------------*/
			$this->Insert_1('image',
								array('name', 'product_id'),
								array(&$img_name, &$product_id),
								'si'
				);
			/*---------------------------------------*/
			if ($subImage) {
	...
	/*---------------- Danh mục tin tức --------------------*/
		...
		public function deleteNewsCat(){
			...
			if ($newscat_id == 75 || $newscat_id == 74 || $newscat_id == 73) {
				echo json_encode(array('error'=>'Mục này không được xóa.'));die;
			}
